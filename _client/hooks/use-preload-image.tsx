...
import { usePreloadedImages } from "_client/stores/preloaded-images-store";
...
export const usePreloadImage: UsePreloadImageHook = () => {
  ...
  return useCallback(({ src, width, quality = 75 }) => {
    ...
    if (preloadedImages.includes(src.replace(/^\/\//, "https://"))) {
      return;
    }
    setPreloadedImages((currentPreloadImages) => [
      ...currentPreloadImages,
      src.replace(/^\/\//, "https://"),
    ]);
